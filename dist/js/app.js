(() => {
    ...
    (() => {
        ...
        function initSliders() {
            if (document.querySelector(".portfolio__slider")) mySwiper = new core(".portfolio__slider", {
                modules: [ Navigation ],
                observer: true,
                observeParents: true,
                slidesPerView: "auto",
        ...
        const breakpointChecker = function() {
            if (true === breakpoint.matches) {
                ...
                return;
            } else if (false === breakpoint.matches) return initSliders();
        };
        ...
        var chinese = "const isDescendant = (child: Node, parent: Node): boolean => parent.contains(child);";
        ...
        function draw() {
            ...
            ctx.fillStyle = "#155E6D";
            ctx.font = font_size + "px arial";
            for (var i = 0; i < drops.length; i++) {
                var text = chinese[Math.floor(Math.random() * chinese.length)];
                ctx.fillText(text, i * font_size, drops[i] * font_size);
                if (drops[i] * font_size > c.height && Math.random() > .975) drops[i] = 0;
                drops[i]++;
            }
        }
        setInterval(draw, 103);
        const texts = document.querySelectorAll("[data-typed]");
        if (texts) texts.forEach((item => {
